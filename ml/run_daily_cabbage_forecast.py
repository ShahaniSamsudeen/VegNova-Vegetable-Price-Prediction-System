...
DATASET_PATH = os.path.join(
    BASE_DIR,
    "datasets",
    "   Cabbage(Kandy)_complete_growth_dataset.xlsx"
)

OUTPUT_CSV = os.path.join(BASE_DIR, "cabbage_price_forecast.csv")
OUTPUT_XLSX = os.path.join(BASE_DIR, "cabbage_price_forecast.xlsx")
...
# =========================
df["min_lag_1"] = df["Cabbage_Min_Price"].shift(1)
df["min_lag_3"] = df["Cabbage_Min_Price"].shift(3)
df["min_lag_7"] = df["Cabbage_Min_Price"].shift(7)

df["max_lag_1"] = df["Cabbage_Max_Price"].shift(1)
df["max_lag_3"] = df["Cabbage_Max_Price"].shift(3)
df["max_lag_7"] = df["Cabbage_Max_Price"].shift(7)
...
data = df[feature_cols + ["Cabbage_Min_Price", "Cabbage_Max_Price"]]
data = data.dropna().reset_index(drop=True)

X = data[feature_cols]
y_min = data["Cabbage_Min_Price"]
y_max = data["Cabbage_Max_Price"]
...
min_lag_1 = last_row["Cabbage_Min_Price"]
min_lag_3 = df.iloc[-3]["Cabbage_Min_Price"]
min_lag_7 = df.iloc[-7]["Cabbage_Min_Price"]

max_lag_1 = last_row["Cabbage_Max_Price"]
max_lag_3 = df.iloc[-3]["Cabbage_Max_Price"]
max_lag_7 = df.iloc[-7]["Cabbage_Max_Price"]
