...
# =========================
# 2. PATH CONFIGURATION (VERY IMPORTANT)
# =========================
...
DATASET_PATH = os.path.join(
    BASE_DIR,
    "datasets",
    "knolkhol_complete_growth_dataset.xlsx"
)

OUTPUT_CSV = os.path.join(BASE_DIR, "knolkhol_price_forecast.csv")
OUTPUT_XLSX = os.path.join(BASE_DIR, "knolkhol_price_forecast.xlsx")
...
# =========================
df["min_lag_1"] = df["Knolkhol_Min_Price"].shift(1)
df["min_lag_3"] = df["Knolkhol_Min_Price"].shift(3)
df["min_lag_7"] = df["Knolkhol_Min_Price"].shift(7)

df["max_lag_1"] = df["Knolkhol_Max_Price"].shift(1)
df["max_lag_3"] = df["Knolkhol_Max_Price"].shift(3)
df["max_lag_7"] = df["Knolkhol_Max_Price"].shift(7)
...
data = df[feature_cols + ["Knolkhol_Min_Price", "Knolkhol_Max_Price"]]
data = data.dropna().reset_index(drop=True)

X = data[feature_cols]
y_min = data["Knolkhol_Min_Price"]
y_max = data["Knolkhol_Max_Price"]
...
# =========================
 # Start forecast from TODAY (dynamic)
...
min_lag_1 = last_row["Knolkhol_Min_Price"]
min_lag_3 = df.iloc[-3]["Knolkhol_Min_Price"]
min_lag_7 = df.iloc[-7]["Knolkhol_Min_Price"]

max_lag_1 = last_row["Knolkhol_Max_Price"]
max_lag_3 = df.iloc[-3]["Knolkhol_Max_Price"]
max_lag_7 = df.iloc[-7]["Knolkhol_Max_Price"]
