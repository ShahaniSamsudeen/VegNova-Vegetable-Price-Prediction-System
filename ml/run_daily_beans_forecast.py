...
DATASET_PATH = os.path.join(
    BASE_DIR,
    "datasets",
    "beans_complete_growth_dataset.xlsx"
)

OUTPUT_CSV = os.path.join(BASE_DIR, "beans_price_forecast.csv")
OUTPUT_XLSX = os.path.join(BASE_DIR, "beans_price_forecast.xlsx")
...
# =========================
df["min_lag_1"] = df["Beans_Min_Price"].shift(1)
df["min_lag_3"] = df["Beans_Min_Price"].shift(3)
df["min_lag_7"] = df["Beans_Min_Price"].shift(7)

df["max_lag_1"] = df["Beans_Max_Price"].shift(1)
df["max_lag_3"] = df["Beans_Max_Price"].shift(3)
df["max_lag_7"] = df["Beans_Max_Price"].shift(7)
...
data = df[feature_cols + ["Beans_Min_Price", "Beans_Max_Price"]]
data = data.dropna().reset_index(drop=True)

X = data[feature_cols]
y_min = data["Beans_Min_Price"]
y_max = data["Beans_Max_Price"]
...
min_lag_1 = last_row["Beans_Min_Price"]
min_lag_3 = df.iloc[-3]["Beans_Min_Price"]
min_lag_7 = df.iloc[-7]["Beans_Min_Price"]

max_lag_1 = last_row["Beans_Max_Price"]
max_lag_3 = df.iloc[-3]["Beans_Max_Price"]
max_lag_7 = df.iloc[-7]["Beans_Max_Price"]
