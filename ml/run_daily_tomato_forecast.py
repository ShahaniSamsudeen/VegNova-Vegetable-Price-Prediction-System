...
DATASET_PATH = os.path.join(
    BASE_DIR,
    "datasets",
    "tomato_complete_growth_dataset.xlsx"
)

OUTPUT_CSV = os.path.join(BASE_DIR, "tomato_price_forecast.csv")
OUTPUT_XLSX = os.path.join(BASE_DIR, "tomato_price_forecast.xlsx")
...
# =========================
df["min_lag_1"] = df["Tomato_Min_Price"].shift(1)
df["min_lag_3"] = df["Tomato_Min_Price"].shift(3)
df["min_lag_7"] = df["Tomato_Min_Price"].shift(7)

df["max_lag_1"] = df["Tomato_Max_Price"].shift(1)
df["max_lag_3"] = df["Tomato_Max_Price"].shift(3)
df["max_lag_7"] = df["Tomato_Max_Price"].shift(7)
...
data = df[feature_cols + ["Tomato_Min_Price", "Tomato_Max_Price"]]
data = data.dropna().reset_index(drop=True)

X = data[feature_cols]
y_min = data["Tomato_Min_Price"]
y_max = data["Tomato_Max_Price"]
...
min_lag_1 = last_row["Tomato_Min_Price"]
min_lag_3 = df.iloc[-3]["Tomato_Min_Price"]
min_lag_7 = df.iloc[-7]["Tomato_Min_Price"]

max_lag_1 = last_row["Tomato_Max_Price"]
max_lag_3 = df.iloc[-3]["Tomato_Max_Price"]
max_lag_7 = df.iloc[-7]["Tomato_Max_Price"]
